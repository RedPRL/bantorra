...
type resolver_arguments = BantorraBasis.Marshal.value
...
val make :
  ...
  (cur_root:string -> resolver_arguments -> string option) ->
  t
(** [make ?fast_checker ?args_dumper resolver] creates a new resolver
    that can be used in {!val:Manager.init}.

    @param resolver [resolver ~cur_root args] is responsible for finding the root of the library specified by [args]. A library manager will feed the resolver with unparsed [resolver_arguments] field in an anchor file. (See {!module:Manager}.) The return value is [None] if the resolution fails.
...
val resolve : t -> cur_root:string -> resolver_arguments -> string
val resolve_opt : t -> cur_root:string -> resolver_arguments -> string option
val fast_check : t -> cur_root:string -> resolver_arguments -> bool
val dump_args : t -> cur_root:string -> resolver_arguments -> string
