...
(** [make ?fast_checker route] creates a new router that can be used in {!val:Manager.init}.
    ...
    @param route [route ~starting_dir ~arg] is responsible for finding the root of the library specified by [arg]. A library manager will feed the router with unparsed [router_argument] field in an anchor file as [arg] and the root of the mounting library as [starting_dir]. (See {!module:Manager}.)
