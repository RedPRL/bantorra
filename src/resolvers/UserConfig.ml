...
let library_in_use : (string, filepath) Hashtbl.t = Hashtbl.create 10
...
let read ~app_name ~config =
  ...
  | Some conf -> conf
  | None ->
    let conf = try deserialize @@ Marshal.read_json filepath with _ -> default_config in
...
let unsafe_write ~app_name ~config conf =
  let filepath = config_filepath ~app_name ~config in
  Marshal.unsafe_write_json ~minify:false filepath @@ serialize conf;
  Hashtbl.replace cache filepath conf
...
let resolver ~app_name ~config =
  ...
  try
    let conf = read ~app_name ~config in
