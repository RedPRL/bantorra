...
(** [resolver ~crate_root] downloads git repositories, when requested, into the directory [crate_root] and resolve the URL into a path pointing to a local copy of the library. The resolver assumes the directory already exists and it has total control over the directory at [crate_root] during the lifespan of the library manager. *)
