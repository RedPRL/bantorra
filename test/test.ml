...
let db = Store.init ~root:"/tmp/bantorra/testing"
let () = print_endline "Adding an item..."
let digest = Store.replace_item db ~key:`Null ~value:(`A [])
let () = print_endline "Finding an item without checking its digest..."
let () =
  match Store.find_item_opt db ~key:`Null ~digest:None with
  | Some `A [] -> ()
  | _ -> assert false
let () = print_endline "Finding an item with the correct digest..."
let () =
  match Store.find_item_opt db ~key:`Null ~digest:(Some digest)  with
  | Some `A [] -> ()
  | _ -> assert false
let () = print_endline "Finding an item with a wrong digest..."
let () =
  match Store.find_item_opt db ~key:`Null ~digest:(Some "123") with
  | None -> ()
  | _ -> assert false
let () = print_endline "Saving the database state..."
let () = Store.save_state db
